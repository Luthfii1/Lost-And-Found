...
const NavbarChoice = () => {
  ...
  if (isMobile) {
      return (
        <div className={`fixed top-0 left-0 right-0 justify-evenly sm:left-[20rem] text-white bg-black rounded-b-2xl ${showNavbar ? 'flex' : 'hidden'}`}>
  ...
  return (
    <div className={`fixed top-0 right-0 justify-evenly sm:left-[20rem] text-white bg-black rounded-b-2xl`}>
        <div className='max-w-[1240px] flex '>
            {/* Create 4 Buttons for All, Lost, Found, and Share justify between*/}
            {/* I want to set this 4 button with eventually */}

            <div className='flex items-center place-items-center justify-evenly h-20 px-4 space-x-5'>
            <button
                className={`p-2 w-16 rounded-md font-semibold ${
                activeButton === 'All' ? 'bg-yellow text-black' : 'bg-slate-400 text-white'
                }`}
