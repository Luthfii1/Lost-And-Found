...
const Post = () => {
  ...
  const onSubmitForm = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/post/${localStorage.getItem('user_id')}`, {
        method: 'POST',
        headers: {
      ...
      {showForm && (
        ...
        >
          ...
          <div className='flex justify-between items-center px-4 pt-4'>
            <div className='flex items-center'>
              ...
              <div className='flex text-center'>
                <div
                  className={`bg-[#717978] hover:cursor-pointer hover:bg-[#ff6b00] h-10 w-14 justify-items-center rounded-l-lg text-center text-white flex items-center ${
                    selectedOption === 'Lost' ? 'bg-[#ff6b00]' : ''
                  }`}
                  onClick={() => {
                ...
                <div
                  className={`bg-[#717978] hover:cursor-pointer hover:bg-[#ffd369] h-10 w-14 justify-items-center rounded-r-lg text-center text-white flex items-center ${
                    selectedOption === 'Found' ? 'bg-[#ffd369]' : ''
                  }`}
                  onClick={() => {
          ...
          <div className='flex justify-between items-center px-4 pt-2 rounded-b-lg pb-3 bg-yellow'>
            ...
            <div className='flex items-center'>
              <FaTelegramPlane className='h-6 hover:cursor-pointer w-6 mr-2 text-black' />
            </div>
          </div>
        </form>
      )}
